...
This guide provides a unified reference for the entire **Packet03 (Packet Redirection)** module in the XDP tutorial. It transitions from in-driver packet reflection (`XDP_TX`) to dynamic interface-to-interface redirection using BPF `DEVMAP` and `HASH` maps, culminating in an autonomous Layer-3 router powered by the Linux Kernel Forwarding Information Base (`bpf_fib_lookup`).

---

## Part 1: Physical Testbed Architecture & Addressing
...
### Complete Addressing & Next-Hop Routing Schema
...
## Part 2: Module Progression & Key Differences
...
## Part 3: Host & Interface Preparation (Pre-requisites)

Run these setup scripts across the respective nodes before attaching XDP programs.

### 1. Jetson (`eth0`) Host Setup
...
### 3. Middlebox Base Network Configuration (`xdpserver`)
...
## Part 4: Assignment 1 — ICMP Echo Responder via `XDP_TX`

### Objective

Build an in-driver ICMP echo responder directly at the XDP driver hook. The program intercepts incoming ICMP Echo Requests (ping), swaps Ethernet MACs and IP addresses, changes ICMP Type from 8 (Request) to 0 (Reply), recalculates the checksum, and returns `XDP_TX` to reflect the frame immediately back out the same interface without hitting the host kernel stack.
...
* **Expected Result:** Instant ICMP Echo Replies return to the Jetson; `xdp_stats` on the middlebox increments the `XDP_TX` counter column.
...
## Part 5: Assignment 2 — Static Interface Redirection (`xdp_redirect`)

### Objective

Redirect incoming frames from an ingress physical port (`ens8f0`) directly out of an egress physical port (`ens8f1`) using `bpf_redirect(ifindex, 0)`. The program updates the Ethernet destination MAC to the next hop and forwards raw frames without traversing kernel IP routing stacks.
...
* **Expected Result:** `tcpdump` on the laptop captures incoming frames forwarded across the middlebox; `xdp_stats` on `ens8f0` increments under `XDP_REDIRECT`.
...
## Part 6: Assignment 3 — Dynamic Multi-Port Redirection (`xdp_redirect_map`)

### Objective

Eliminate hardcoded interface indices and static MAC addresses by using BPF maps (`BPF_MAP_TYPE_DEVMAP` named `tx_port` and `BPF_MAP_TYPE_HASH` named `redirect_params`). The user-space loader configures dynamic port-forwarding tables across multiple interfaces, allowing full bidirectional packet transmission.
...
* **Expected Result:** Packets flow bidirectionally between Jetson and Laptop with zero packet loss; both `xdp_stats` monitors increment `XDP_REDIRECT`.
...
## Part 7: Assignment 4 — Full Layer-3 Router via `bpf_fib_lookup` (`xdp_router`)

### Objective

Construct an autonomous, kernel-integrated Layer-3 Router directly at the XDP driver level:

1. Parse Layer-3 IPv4 headers.
2. Query the Linux Forwarding Information Base (FIB) and Neighbor/ARP cache via `bpf_fib_lookup()`.
3. When `bpf_fib_lookup()` returns `BPF_FIB_LKUP_RET_SUCCESS` (0):
* Decrement the IPv4 `ttl` field and update the IP header checksum with `ip_decrease_ttl()`.
* Overwrite destination MAC (`eth->h_dest`) with the next-hop MAC (`fib_params.dmac`).
* Overwrite source MAC (`eth->h_source`) with the egress interface MAC (`fib_params.smac`).
* Forward the packet out of the resolved egress port (`fib_params.ifindex`) using `bpf_redirect()`.


4. Fall back to `XDP_PASS` for non-routable traffic or unpopulated neighbor entries so the kernel stack can handle ARP discovery.

### System & Kernel FIB Preparation (Middlebox)
...
### Router Loading & Execution Steps
...
## Part 8: Troubleshooting & Diagnostics Guide
...
## Part 9: Comprehensive Command Reference
